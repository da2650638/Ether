...
void Sandbox2D::OnUpdate(Ether::Timestep ts)
{
...

	{
		...
		Ether::Renderer2D::DrawRotatedQuad({ -0.0f, 0.0f }, { 1.0f, 1.0f }, rotation, m_Texture2, 1.0f);
		...
		for (float y = -4.75; y < 5.25; y += 0.5)
		{
			...
			{
				Ether::Renderer2D::DrawQuad({ x, y, 0.0f }, { 0.45f, 0.45f }, { (x + 4.75) / 10.0f, (y + 4.75) / 10.0f, 0.3f, 1.0f });
			}
		}
		Ether::Renderer2D::EndScene();
	}	
